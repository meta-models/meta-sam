...
const alphabet = (() => {
  ...
  for (let code = 0x21; code <= 0x7e; code += 1) {
    ...
    if (!excluded.has(character)) value += character;
  }
  return value;
})();
const radix = 85;
...
function invalid(message: string, cause?: unknown): InvalidSegmentationMaskError {
  return new InvalidSegmentationMaskError(
    message,
...
function pack(input: Uint8Array): string {
  let length = input.length;
  const prefix = new Array<string>(prefixLength);
  for (let index = prefixLength - 1; index >= 0; index -= 1) {
    prefix[index] = alphabet[length % radix]!;
    length = Math.floor(length / radix);
  }
  if (length !== 0) throw invalid('Mask payload is too large.');
  ...
  for (; offset < full; offset += 4) {
    ...
    for (const encoded of digits) output.push(alphabet[encoded]!);
  }
...
function unpack(payload: string): Uint8Array {
  ...
  const expected =
    prefixLength + Math.floor(length / 4) * 5 + (remainder === 0 ? 0 : remainder + 1);
  ...
  for (; destination < full; destination += 4) {
    ...
    if (value > mask32) throw invalid('Mask payload contains an out-of-range group.');
    source += 5;
  ...
  if (remainder > 0) {
    ...
    if (value > mask32) throw invalid('Mask payload contains an out-of-range tail.');
...
class RangeDecoder {
  ...
  public constructor(input: Uint8Array) {
    if (input.length < 4) throw invalid('Mask payload is missing its finalization.');
    this.#input = input;
    for (let index = 0; index < 4; index += 1) {
  ...
  public frequency(total: number): number {
    this.#scaled = Math.floor(this.#range / total);
    if (this.#scaled === 0) throw invalid('Mask payload has invalid coding state.');
    const value = Math.floor(((this.#code - this.#low) >>> 0) / this.#scaled);
    return value >= total ? total - 1 : value;
  }

  public getFreq(total: number): number {
    return this.frequency(total);
  }

  public decode(cumulative: number, frequency: number): void {
...
function getOrderTwo(map: Map<number, Uint16Array>, key: number): Uint16Array {
...
function predictByte(
  ...
  if (mode === 3) return (left + up) >> 1;
  const upperLeft = x > 0 && y > 0 ? output[offset - width - 1]! : 0;
  if (mode === 0) return paeth(left, up, upperLeft);
  if (upperLeft >= Math.max(left, up)) return Math.min(left, up);
  if (upperLeft <= Math.min(left, up)) return Math.max(left, up);
  return left + up - upperLeft;
}

function unzig(value: number, prediction: number): number {
...
function zeroContext(
  ...
  const upperRight =
    y > 0 && x < width - 1 ? (residuals[offset - width + 1] === 0 ? 1 : 0) : 1;
  return left | (up << 1) | (upperLeft << 2) | (upperRight << 3);
}

function decodeLosslessRaster(
  payload: string,
  width: number,
  height: number,
): Uint8Array {
  ...
  if (packed.length < 5) throw invalid('Lossless mask payload is truncated.');
  ...
  const orderTwo = new Map<number, Uint16Array>();
  let previous = 0;
  ...
  for (let offset = 0; offset < length; offset += 1) {
    ...
    if (bit === 1) {
      ...
      const total =
        second[totalIndex]! + orderOneTotals[previous]! + orderZeroTotal - excludedZero;
      ...
      while (cumulative + frequency <= target) {
        ...
        symbol += 1;
        if (symbol >= symbols) throw invalid('Lossless mask payload is malformed.');
        frequency =
...
function updateCounts(counts: Uint16Array, offset: number, bit: number): void {
...
function encodeRaster(input: Uint8Array, width: number, height: number): string {
  ...
  for (let y = 0; y < height; y += 1) {
    ...
    for (let x = 0; x < width; x += 1) {
      ...
      const context = contextAt(
        padded,
        position,
        position - paddedWidth,
        position - 2 * paddedWidth,
      );
      const offset = context << 1;
      const zero = counts[offset]!;
      const one = counts[offset + 1]!;
      const bit = input[y * width + x]!;
      if (bit !== 0 && bit !== 1) throw invalid('Decoded mask is not binary.');
      if (bit === 0) encoder.encode(0, zero, zero + one);
      else encoder.encode(zero, one, zero + one);
      padded[position] = bit;
...
function decodeRaster(payload: string, width: number, height: number): Uint8Array {
  ...
  for (let y = 0; y < height; y += 1) {
    ...
    for (let x = 0; x < width; x += 1) {
      ...
      if (bit === 0) decoder.decode(0, zero);
      else decoder.decode(zero, one);
      padded[position] = bit;
...
export function encodeSegmentationMask(
  raster: Uint8Array,
  width: number,
  height: number,
): SegmentationMask {
 ...
 * only 0 and 1 values.
 */
export function decodeMaskToRaster(mask: SegmentationMask): Uint8Array {
