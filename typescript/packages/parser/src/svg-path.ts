...
export function decodeToSVGPath(rleObjs: RLEObject | Array<RLEObject>): Array<string> {
...
function traceContours(mask: Uint8Array, height: number, width: number): string {
  const getPixel = (row: number, col: number): number => {
...
function connectSegments(
  ...
  for (const [startKey, neighbors] of adjacency) {
    ...
    while (true) {
      ...
      const currentNeighborsFiltered = currentNeighbors.filter(
        (n) => pointKey(n.x, n.y) !== nextKey,
      );
      ...
      if (nextNeighbors != null) {
        const nextNeighborsFiltered = nextNeighbors.filter(
          (n) => pointKey(n.x, n.y) !== currentKey,
        );
