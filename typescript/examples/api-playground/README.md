...
An interactive playground for the SAM 3 Responses API built on `@meta-sam/parser`, `@meta-sam/graphics`, `@meta-sam/video`, and `@meta-sam/react`. One surface handles both images and video: drop a file or pick an example, name what to segment, and watch the streamed output land on a single Canvas. Each mask is drawn as a translucent fill with a crisp contour in the object's color; the inspector's Outlines switch turns that contour off. The canvas labels each box's top-left corner with the run's noun phrase, the object ID, and, when the model returns the optional `c` confidence, the confidence in parentheses, as `pillow 3 (0.945)`; the legend and records show the confidence too, and records without `c` show no value. For live image runs, the Filter by score switch shows a Score threshold slider from 0 to 1; its value is sent as `metadata.score_threshold`, so the API returns only objects that score at least that value. With the switch off, no threshold is sent. Video runs and replays do not send it. For live image and video runs, the Include confidence switch, on by default, sends `metadata.include_confidence` as `"true"` to ask the model for the optional `c` values; with the switch off, the key is not sent. Replays never send it. The right-hand inspector shows the per-object legend (colors match the overlay), parsed records, the raw `output_text` stream with arrival times, and the concatenated raw output. The header independently controls light/dark/system mode and the six bundled Astryx visual themes; the visual theme is restored from local browser storage.
